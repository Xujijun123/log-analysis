...
def test_model(test_x):
    model = load_model('./model/LightLog/model/E_TCN_GAP.h5')
    y_pred = model.predict(test_x, batch_size=512)
    return y_pred

# Data handling
start_data = time.clock()
test_path = './model/LightLog/data/log_test_2000.csv'
...
datas.to_csv('./model/LightLog/result/result_HDFS',index=False,header=False)
