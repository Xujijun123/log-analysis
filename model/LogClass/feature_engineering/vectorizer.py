...
def log_to_vector(inputData, vocabulary):
    ...
    result = []
    for line in inputData:
        vector = np.zeros(len(vocabulary), dtype=np.int32)  # Initialize a vector with zeros of integer type
        token_list = tokenize(line)
        if token_list:
            for token in token_list:
                if token in vocabulary:
                    vector[vocabulary[token]] += 1  # Increment count of tokens
...
def create_invf_vector(inputVector, invf_dict, vocabulary):
    ...
    for line in inputVector:
        ...
        for token_index in line:
            if isinstance(token_index, int) and token_index < len(vocabulary):
                if token_index in invf_dict:
                    cur_tfinvf[token_index] = float(count_dict[token_index]) * invf_dict[token_index]
